...
关键数据决策：**8760h 逐时数据统一走 TimescaleDB 超表**，其中 `DynamicLoadHourly`（逐时风量/负荷结果，PRD §19 别名 `RoomHourlyLoadResult`）作为 TimescaleDB 超表按时间 `range` 分区存储；气象逐时数据 `WeatherRecord` 同样以超表存放；其余关系型数据（项目/建筑/楼层/功能区域、`RatioCoefficient` 比例系数表、汇总结果等）存于 PostgreSQL 主库。
...
```
   ...
   | 关系表:       | | DynamicLoad   | |           | |               | | 气象上传 |
   | Project/      | | Hourly,       | |           | | 消费 Redis    | |         |
   | Room/         | | WeatherRecord | |           | | 队列,结果     | |         |
   | RatioCoeff/   | | (8760h 逐时,  | |           | | 落 PG/TSDB    | |         |
   | 汇总结果      | |  时间 range)  | |           | |               | |         |
   +---------------+ +---------------+ +-----------+ +---------------+ +---------+
```

> 表名一律 PascalCase（`Project` / `Room` / `RatioCoefficient` / `DynamicLoadHourly` / `WeatherRecord`），字段保持 snake_case。逐时类超表统一按**时间 range 分区**（不再出现 `room_id + 时间` 与纯时间两种分区键混用的矛盾，详见 §5）。
...
flowchart LR
  ...
  A1 --> TSDB[(TimescaleDB 超表<br/>DynamicLoadHourly / WeatherRecord)]
...
| TimescaleDB | timescale/timescaledb 镜像（PG16 内核） | 同 dev（timescale/timescaledb 镜像） | timescale/timescaledb 镜像，超表分区 |
...
> **TimescaleDB 镜像要求（三套环境一致）**：dev / test / prod 的数据库容器**统一使用 `timescale/timescaledb:<version>-pg16` 镜像**（如 `timescale/timescaledb:2.14.2-pg16`），**不使用纯 `postgres:16`**。原因：超表（hypertable）依赖 TimescaleDB 扩展，纯 postgres 镜像无法创建超表，会导致 dev 可跑、test/prod 失败的环境漂移。若需简化小型部署，可在同一 timescaledb 实例内既存关系表又存超表（详见 §3 说明）。

---

## 3. docker-compose 要点（复用 §8.1 并补全）

在架构 §8.1 片段基础上补全：新增 `timescaledb`、`nginx`、`celery-beat`、`pgbouncer`、`minio`；统一 env_file；增加 healthcheck 与 `depends_on: condition`；补全持久化卷。
...
services:
  ...
  db:
    # 统一使用 timescale/timescaledb 镜像（PG16 内核），三套环境一致；
    # 纯 postgres:16 不可用——关系表与超表（hypertable）可共存于同一实例。
  ...
  minio:
    ...
    env_file: [.env.minio]   # MINIO_ROOT_USER / MINIO_ROOT_PASSWORD
...
> 说明：小型单机部署可将 `timescaledb` 与 `db` 合并为**同一个 `timescale/timescaledb` 实例**（关系表与超表共存，db 即已使用 timescaledb 镜像）；两者分离仅为读写 / 容量隔离预留。**禁止用纯 `postgres:16` 替代**（无法创建超表，见 §2）。
...
- **PostgreSQL 主库**：命名卷 `pgdata` 持久化；关系表包括 `Project / Building / Floor / Room / RatioCoefficient / RoomLoadResult / RoomLoadSummary / LoadSummary / Settings / User` 等（架构 §4.1，表名 PascalCase）。
- **TimescaleDB 超表**：作为 PG 扩展，对 8760h 逐时数据**统一按时间 `range` 分区**（chunk 默认 1 个月，超大数据量场景可收紧到 7 天）：
  - `DynamicLoadHourly`（逐时负荷，PRD §19 别名 `RoomHourlyLoadResult`）：**明确走 TimescaleDB 超表**，存储每个功能区域逐时风量/负荷（约 26280 行/功能区域，F8-014）。
  - `WeatherRecord`（气象逐时）：同样以超表存放，支撑 F6-001 连续 3 年逐时气象与动态仿真。
  - **分区键统一原则**：所有逐时类超表**只按时间列 `range` 分区**；不再出现「`room_id + 时间`」与「纯时间」两种分区键混用的旧写法。`room_id` 等维度列以普通索引或复合索引支撑查询过滤，不作为分区键，避免分区数随房间数爆炸（1000+ 房间 × 36 个月 = 数万 chunk 的失控风险）。
  - 超表随 PG 主库卷一并备份（见 §7）。
